...
tags:
  ...
  - genomica
  - proteinas
translation_source_hash: c88b5b5d38a55d2725d115db19ad97e33c5c7e571543a4819b4e5469576a23bf
---
...
Scientific researchers deal with dozens of data sources—each with its own schema and query language—file formats that require specific pipelines and viewers, and an arsenal of tools including PubMed, Jupyter, R, and cluster terminals. Moving between these systems consumes time and introduces errors.

Claude Science consolidates these tools into a single environment where scientists can conduct all stages of their work: from literature analysis to multi-step research execution, and the production of publication-ready artifacts, figures, and manuscripts.
...
Each generated figure comes with the exact code, environment description, a natural language explanation of how it was created, and the full conversation history. The user can request changes in natural language — "remove grid lines" or "change axis to log scale" — and the agent edits its own code.
...
1. Develops a plan
2. Asks for authorization before accessing new resources
3. Allows reviewing or revoking any decision
4. Submits the job to the compute resources the lab already uses (HPC via SSH or Modal for on-demand computing)
...
Claude Science represents a significant bet by Anthropic in the scientific research market — a segment that companies like Google (with AlphaFold) and NVIDIA (with BioNeMo) are already competing in. The difference lies in the **integrated agent workbench** approach: instead of offering isolated tools for protein folding or genomic analysis, Claude Science unifies the entire research workflow in a natural language-controlled environment, with built-in reproducibility and auditing.

For the Brazilian research ecosystem, availability via remote access (SSH/HPC) and support for academic labs with differentiated pricing are relevant points — institutions like CNPEM, LNCC, and state university networks will be able to test the platform without requiring initial investment in additional infrastructure.

The simultaneous launch of Claude Sonnet 5 — which Anthropic describes as "frontier performance in coding, agents, and professional-scale work" — suggests that Claude Science is powered by the company's latest model, which could give it competitive advantages in tasks requiring long reasoning and scientific code manipulation.
