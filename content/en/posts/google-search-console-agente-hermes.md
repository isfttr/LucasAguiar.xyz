...
tags:
  ...
  - docker
  - mcp
translation_source_hash: 9fa4330d9b5892ea8b650f4cb6a52813883a4f915d85bf999825e883fe80152d
---
I had been following **Google Search Console** for a while, but the workflow was manual and boring: open the panel, copy the queries, paste them into my agent. It can be done much better. In this tutorial, I show how to connect Search Console directly to **Hermes** — the AI agent running on my homelab — using an **MCP** server, so it reads clicks, impressions, and queries **via API**. I'll cover both what the creator's repository already delivers out of the box and the adjustments I had to make to run everything inside Docker, which is where the official documentation stops helping you.

## What is MCP and what gsc-mcp delivers

The [Model Context Protocol (MCP)](https://modelcontextprotocol.io) is an open standard for giving AI agents "tools" — each MCP server exposes functions the agent can call. For Search Console there is [`google-search-console-mcp`](https://github.com/ncosentino/google-search-console-mcp), by **Nick Cosentino** (Dev Leader), under the MIT license.
...
- **Three tools, all read-only**: `list_sites` (lists accessible properties), `list_sitemaps` (submitted sitemaps and status), and `query_search_analytics` (clicks, impressions, CTR, and average position — with dimensions like query, page, country, and device, up to 25 thousand rows and 16 months of history).
...
4. In [Google Search Console](https://search.google.com/search-console), go to **Settings → Users and permissions → Add user**, paste that email and give read permission, **for each property** the agent should access.
...
- The binary **and** the JSON key must exist *inside the container's filesystem* — leaving them only on the host is not enough.
...
## Step by step (with adjustments for Docker)
...
**3. Mount the folder into the container (read-only).** In the Hermes service, inside `volumes:`:
...
Before using it, it's worth verifying in three layers — this separates "problem with Google" from "problem with Hermes":
...
3. **The API responds:** ask the agent to run `list_sites`. The response looks like this:

```json
{"sites":[{"siteUrl":"sc-domain:exemplo.com","permissionLevel":"siteFullUser"}]}
```

If it comes back **empty** or with a permission error, the problem is almost always step 4 of the prerequisites (the service account was not added to the property) — it's not Hermes. Tip from someone who learned the hard way: you can test the binary **on its own** by sending an MCP handshake via `stdin` (`initialize` → `tools/call` `list_sites`), isolating Google from the agent before blaming the integration.
...
With this, the same agent I set up when [I created my AI assistant locally]({{< relref "posts/creating-my-ai-assistant-locally/" >}}) now answers questions like *"which queries brought the most clicks in the last 28 days?"* on its own — pulling data directly from the API, without me copying and pasting anything. Because they are read-only tools, it is safe to keep it on in daily use, and any future adjustments can be applied with a `/reload-mcp`.
